...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs, tv_mask=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, objs, tv_mask)
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    ...
    def forward(self, input, tv_mask=None):
        ...
        h = self.middle_block(h, emb, context, objs, tv_mask)
